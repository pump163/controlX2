...
class HistoryLogFetcherIntegrationTest {
    ...
    fun orphanedHoleBelowLatestSeq_isHealedOnNextStatus() = runBlocking {
        // Regression test for the "history sync stops at 99%" report.
        //
        // A fetch interrupted by a BLE disconnect left a small hole in the middle of the
        // retained window, but the DB's newest seq id is already well above the hole. The
        // fetcher must re-scan the whole window and re-request the hole, instead of only
        // fetching entries newer than the newest stored one (which would orphan the hole
        // permanently and pin the progress bar just under 100%).
        val pumpLastSeq = 8000L
        val catchupStart = pumpLastSeq - InitialHistoryLogCount // 3000
        // The DB already covers the whole top window EXCEPT a 10-entry hole sitting well
        // below the newest stored entry — i.e. dbLatestId (8000) is far above the hole.
        insertExistingLogs(catchupStart..7000L)   // 3000..7000
        // hole: 7001..7010 missing
        insertExistingLogs(7011L..pumpLastSeq)     // 7011..8000
        val fetcher = createAutoRespondingFetcher()
        val status = makeStatusResponse(
            numEntries = pumpLastSeq, firstSeqNum = 1, lastSeqNum = pumpLastSeq
        )
        val scope = CoroutineScope(Dispatchers.Default + Job())
        fetcher.onStatusResponse(status, scope)
        waitForFetchCompletion(scope, timeoutMs = 60_000L)
        // The orphaned hole must have been re-requested...
        val requestedSeqIds = mutableSetOf<Long>()
        sentCommands.forEach { (start, count) ->
            val rangeStart = start - count + 1
            (rangeStart..start).forEach { requestedSeqIds.add(it) }
        ...
        for (id in 7001L..7010L) {
            assertTrue("Hole seqId $id should have been re-requested", id in requestedSeqIds)
        }
        // ...and filled, so the full window is now contiguous.
        val windowIds = repo.getAllIds(TEST_PUMP_SID, catchupStart, pumpLastSeq)
        assertEquals(
            "Whole retained window should be present after healing the hole",
            (pumpLastSeq - catchupStart + 1).toInt(),
            windowIds.size
        )
    }
}
